...
from tensorflow.keras.layers import (
    ...
    Layer, GlobalAveragePooling1D, MaxPooling1D,
    Bidirectional, LSTM
)
...
class DynamicConv1D(Layer):
    def __init__(self, filters, kernel_size, kernel_num=4, **kwargs):
        super().__init__(**kwargs)
        self.filters = filters
        self.kernel_size = kernel_size
        self.kernel_num = kernel_num
        self.global_pool = GlobalAveragePooling1D()
        self.attn_dense = Dense(kernel_num, activation='softmax')

    def build(self, input_shape):
        in_ch = input_shape[-1]
        self.kernels = self.add_weight(
            shape=(self.kernel_num, self.kernel_size, in_ch, self.filters),
            initializer='glorot_uniform',
            trainable=True, name='dyn_kernels'
        )
        self.biases = self.add_weight(
            shape=(self.kernel_num, self.filters),
            initializer='zeros',
            trainable=True, name='dyn_biases'
        )
        super().build(input_shape)

    def call(self, x):
        context = self.global_pool(x)
        attn = self.attn_dense(context)                 # [B, K]
        attn_mean = K.mean(attn, axis=0)                # [K]（简化：跨 batch 平均）
        kern = K.sum(self.kernels * K.reshape(attn_mean, (-1, 1, 1, 1)), axis=0)
        bias = K.sum(self.biases * K.reshape(attn_mean, (-1, 1)), axis=0)
        y = tf.nn.conv1d(x, kern, stride=1, padding='SAME')
        return y + bias
...
def build_model(num_dev, seq_len, num_cls):
    ...
    x = DynamicConv1D(64, kernel_size=3, kernel_num=4)(inp_sig)
    x = MaxPooling1D(2)(x); x = Dropout(0.3)(x)
    x = DynamicConv1D(128, kernel_size=3, kernel_num=4)(x)
    x = MaxPooling1D(2)(x); x = Dropout(0.3)(x)
    x = Bidirectional(LSTM(64, return_sequences=True))(x)
    x = Dropout(0.3)(x)
    x = Attention()(x)
    b_sig = Dense(128, activation='relu')(x)

    inp_time = Input((1,), name='time_input')
    b_time = Dense(16, activation='relu')(inp_time)

    merged = concatenate([b_dev, b_sig, b_time])
    x = Dense(256, activation='relu', name='feature_layer')(merged)  # 特征层命名
    x = Dropout(0.5)(x)
...
def main():
    ...
    plt.tight_layout()
    save_figure('confusion_matrix4.png')
    ...
    plt.tight_layout()
    save_figure('tsne_features4.png')

    model.save('fault_diagnosis_bilstm_dynconv_attention.keras')
    print("\n模型已保存: fault_diagnosis_bilstm_dynconv_attention.keras")
